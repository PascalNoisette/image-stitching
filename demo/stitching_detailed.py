...
def main():
    ...
    if args.timelapse is not None:
        ...
        if args.timelapse == "as_is":
            ...
            print("Bad timelapse method", file=stderr)
            exit(-1)
    else:
        timelapse = False
    ...
    for name in img_names:
        ...
        if full_img is None:
            print("Cannot read image ", name, file=stderr)
            exit(-1)
    ...
    if num_images < 2:
        print("Need more images", file=stderr)
        exit(-1)

    estimator = ESTIMATOR_CHOICES[args.estimator]()
    b, cameras = estimator.apply(features, p, None)
    if not b:
        print("Homography estimation failed.", file=stderr)
        exit(-1)
    for cam in cameras:
    ...
    if not b:
        print("Camera parameters adjusting failed.", file=stderr)
        exit(-1)
    focals = []
    for cam in cameras:
